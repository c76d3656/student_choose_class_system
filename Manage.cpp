...
std::map<unsigned int, std::string> course_nature_list = {{1, "公共必修"},
                                                          {2, "学科必修"},
                                                          {3, "学科选修"},
                                                          {4, "专业必修"},
                                                          {5, "专业选修"},
...
bool Manage::remove_class(const int &course_id) {
...
bool
Manage::new_class(const std::string &Course_name, unsigned int Course_nature, unsigned int Total_hours,
...
long long Manage::how_many_student_choose_this_class(const long long &course_id) {
    long long number = total_class_list.size();
    return number;
}

void Manage::target_class_list_show(const int &course_id) {
...
void Manage::who_choose_this_class(const unsigned int &course_id) {
...
long long Manage::how_many_class() {
    long long number = total_class_list.size();
...
//学生
bool Manage::new_student(const long long &student_id, const std::string &student_name, bool gender, unsigned int ages,
...
bool Manage::remove_student(const long long &student_id) {
...
void Manage::target_student_show(const long long &student_id) {
...
bool Manage::student_choose_class(const long long &student_id, const int &course_id) {
...
bool Manage::student_remove_class(const long long &student_id, const int &course_id) {
    auto s = total_student_list.find(student_id);
    if (s == total_student_list.end()) {
        printf("学生学号错误\n");
        return false;
...
void Manage::student_change_information(const long long &student_id) {
    auto s = total_student_list.find(student_id);
    if (s == total_student_list.end()) {
        printf("学生学号错误\n");
        return;
...
long long Manage::how_many_student() {
...
//专业操作
unsigned int Manage::new_major(const std::string &major_name) {
...
//课程性质操作
void Manage::course_nature_list_show() {
...
//void Manage::dropInput() {
//    while (std::cin && std::cin.get() != '\n')continue;
//    std::cin.clear();
//}

unsigned long long Manage::input_solution(const std::string &a) {
    std::hash<std::string> str_hash;
